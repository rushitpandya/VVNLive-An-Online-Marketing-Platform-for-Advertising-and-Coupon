...
function payed_ad(text)
{
	alert(text);
		var adid=text;
//		alert(id);
        $.post("payedad.php", { adid:adid},  
            function(result){  
                //if the result is 1  
                if(result == 1){  
				alert("Payed");
				...
				location.reload();
                    ...
                    //show that the username is NOT available  
					alert("not payed error in some database connectivity");
                }  
        });  
}
</script>
...
{ include 'header.php';?>
	...
	while($row=mysqli_fetch_array($result))
	{
		...
		<div class="row center">
									...
									<?php
									$count=1;
									$query2="SELECT * from adsinformation where confirm=1";
									...
									?>
									<div class="col s3 m7 l5">
										<div class="card-panel large blue lighten-5 center">
														<input type="hidden" id="adsinfoid" name="adsinfoid" value="<?php echo $adsinfoid; ?>"/>
																<center><img src="../images/<?php echo $adsimage;echo "?id=";echo rand();  ?>" class=" materialboxed responsive-img"></center>
																	...
																	<?php
																		...
																		</br><center><a class="white red-text text-darken-3 btn " id="cancelbtn" onclick="cancel_ad(<?php echo $adsinfoid;?>)">Reject<i class="  red-text text-darken-3 material-icons circle right">not_interested</i></a>	
																		<a class="white yellow-text text-darken-3 btn" id="payed" onclick="payed_ad(<?php echo $adsinfoid;?>)">Pay<i class="green-text text-darken-3 material-icons circle right">done</i></a></center>	
																		<?php	
																		}
																		if($confirm==1 && $payment==1)
																		{?>
																		</br><center><a class="white yellow-text text-darken-3 btn disabled" id="confirmed">Verifed<i class="green-text text-darken-3 material-icons circle right">verified_user</i></a>
																		<a class="white yellow-text text-darken-3 btn disabled" id="payed_done" >Payed<i class="green-text text-darken-3 material-icons circle right">done_all</i></a></center>	
																		<?php	
																		}
																		else if($confirm==2)
																		{?>
